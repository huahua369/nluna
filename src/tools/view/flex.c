...
// 分配内存
void* flex_malloc(size_t s)
{
	void* d = malloc(s);
	return d;
}
void* flex_realloc(void* p, size_t s)
{
	void* d = realloc(p, s);
	return d;
}
struct flex_item* flex_item_idx(struct flex_item* it, int idx)
{
	return it + idx;
}
struct flex_item* flex_item_new(int count)
{
	assert(count > 0);
	struct flex_item* item = (struct flex_item*)flex_malloc(sizeof(struct flex_item) * count);
	assert(item != NULL);
	for (size_t i = 0; i < count; i++)
	{
		flex_item_init(&item[i], false);
	}
	item->is_free = true;
	return item;
}
struct flex_item* flex_item_resize(struct flex_item* item, int count, int old_capsize, bool clearall)
{
	assert(count > 0);
	unsigned int c = old_capsize;
	if (old_capsize < 1)
	{
	...
	do
	{
		...
		struct flex_item* t = (struct flex_item*)flex_realloc(item, sizeof(struct flex_item) * (count));
		if (t)
		{
			item = t;
		}
	...
	for (size_t i = c; i < count + c; i++)
	{
		flex_item_init(&item[i], false);
	}
	return item;
}
struct flex_item* flex_item_init(struct flex_item* item, int is_free)
{
	assert(item != NULL);
...
grow_if_needed(struct flex_item* item)
{
	if (item->children.count == item->children.cap) {
		item->children.cap += 24;
		assert(item->children.count < item->children.cap);

		item->children.ary = (struct flex_item**)flex_realloc(item->children.ary,
...
struct flex_item* flex_item_detach(struct flex_item* item, struct flex_item* child)
{
	if (!item || !child || !(item->children.count > 0))return child;
	assert(item->children.count > 0);
	unsigned int index = -1;
	do
	{
		for (unsigned int i = 0; i < item->children.count; i++) {
			if (item->children.ary[i] == child) { index = i; break; }
		}
		if (index == -1)break;
		for (unsigned int i = index; i < item->children.count - 1; i++) {
			item->children.ary[i] = item->children.ary[i + 1];
	...
	struct flex_layout* layout)
{
	...
	if (item->should_order_children && item->children.count > 0) {
		unsigned int* indices = (unsigned int*)flex_malloc(sizeof(unsigned int)
			* item->children.count);
		assert(indices != NULL);
...
static void layout_cleanup(struct flex_layout* layout)
{
...
static void grow_if_needed_layout(struct flex_layout* layout)
{
	if (layout->lines_count == layout->lines_cap) {
		struct flex_layout_line* old = layout->lines;
		layout->lines_cap += 4096 / sizeof(struct flex_layout_line);
		layout->lines = (struct flex_layout_line*)realloc(layout->lines,
			sizeof(struct flex_layout_line) * (layout->lines_cap));
...
layout_items(struct flex_item* item, unsigned int child_begin,
	...
	struct flex_layout* layout, unsigned int last_count)
{
	...
	if (layout->need_lines) {
		grow_if_needed_layout(layout);
		// layout->lines = (struct flex_layout_line*)realloc(layout->lines,sizeof(struct flex_layout_line) * (layout->lines_count + 1));
		assert(layout->lines != NULL);
...
#undef ABSOLUTE_SIZE
		...
		if (layout->wrap) {
			if (layout->flex_dim < child_size) {
				...
				layout_items(item, last_layout_child, i,
					relative_children_count, layout, last_count);
	...
	if (layout->need_lines && layout->lines_count > 0) {
		...
		if (flex_dim > 0) {
			if (!layout_align(item->align_content, flex_dim, layout->lines_count, &pos, &spacing, true))
			{
...
int flex_item_mem_size()
{
	return sizeof(struct flex_item);
}
