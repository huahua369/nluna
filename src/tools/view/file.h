...
#define _MAX_EXT    256 // max. length of extension component
		...
		static bool remove_file(std::string filename)
		{
			...
			if (ret != 0)
			{
				switch (errno)
				{
				case EROFS: printf("文件[%s]为只读文件。\n", filename.c_str()); break;
				case EFAULT: printf("参数[%s] 指针超出可存取内存空间。\n", filename.c_str()); break;
				case ENAMETOOLONG: printf("参数[%s] 太长。\n", filename.c_str()); break;
				case ENOMEM: printf("核心内存不足。文件[%s]\n", filename.c_str()); break;
				case ELOOP: printf("参数[%s] 有过多符号连接问题。\n", filename.c_str()); break;
				case EIO: printf("I/O 存取错误。文件[%s]\n", filename.c_str()); break;
				default:
					break;
				}
			}
